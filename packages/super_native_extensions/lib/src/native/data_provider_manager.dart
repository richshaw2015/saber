...
class DataProviderManagerImpl implements DataProviderManager {
  ...
  Future<dynamic> _onMethodCall(MethodCall call) async {
    if (call.method == 'getLazyData') {
      ...
      if (lazyData != null) {
        return _ValuePromiseResult.ok(await getData(lazyData: lazyData))
            .serialize();
      } else {
  ...
  Object? getData({lazyData = DataRepresentationLazy}) async {
    final data = await lazyData.dataProvider();
    if (_methodChannel == null) {
      return data;
    }
    final preprocessedData = await _methodChannel.invokeMethod<Object>('dataPreprocess', <String, dynamic>{
        'data': data, 
        'format': lazyData.format,
      });
    return preprocessedData != null ? preprocessedData : data;
  }
