...
Future<int> main(List<String> args) async {
  try {
    ...
    // final deps = await oss.listDependencies(pubspecYamlPath: projectPubspecYaml.path, pubspecLockPath: projectPubspecYaml);
    // final pdfrxWasmPackage = [...deps.allDependencies, deps.package].firstWhere((p) => p.name == 'pdfrx');
    // 兼容旧版本实现
    final projectPubspecLock = path.join(projectRoot, 'pubspec.lock');
    final deps = await oss.listDependencies(pubspecLockPath: projectPubspecLock);
